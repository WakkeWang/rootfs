...
ME3630ID=19d2:1476                      
RET=`busybox lsusb | grep $ME3630ID | awk '{print $6}'`                       
if [ "$RET" == "$ME3630ID" ];then     
        rmmod cdc_ether         
        /sbin/modprobe option                           
fi

HUAWEI_909=12d1:15c1                                                                         
HUAWEI_609=12d1:1573                             
HUAWEI_709=12d1:1c25                           
RET1=`busybox lsusb | grep $HUAWEI_909 | awk '{print $6}'`
RET2=`busybox lsusb | grep $HUAWEI_609 | awk '{print $6}'`
RET3=`busybox lsusb | grep $HUAWEI_709 | awk '{print $6}'`
if [ "$RET1" == "$HUAWEI_909" ] || [ "$RET2" == "$HUAWEI_609" ] || [ "$RET3" == "$HUAWEI_709" ];then
        rmmod cdc_ether
fi
...
MDG100=05c6:90b3
RET=`busybox lsusb | grep $MDG100 | awk '{print $6}'`
if [ "$RET" == "$MDG100" ];then
        rmmod rndis_host cdc_ether option
        /sbin/modprobe option cdc_ether rndis_host
fi
...
# Load RS9113_Driver
busybox lsusb | grep "1618:9113" 2> /dev/null
if [ "$?" == "0" ];then
	/usr/local/RS9113_Driver/rs9113_drv_enable.sh sta 
	ifconfig rpine0 up
	ifconfig wlan0 up
	hciconfig hci0 up
fi
# Load RS9116_Driver
busybox lsusb | grep "1618:9116" 2> /dev/null
if [ "$?" == "0" ];then
	/usr/local/RS9116_Driver/rs9116_drv_enable.sh sta 
	ifconfig rpine0 up
	ifconfig wlan0 up
	hciconfig hci0 up
fi
