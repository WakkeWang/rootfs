...
mkdir -p $TMP_PATH
cp -a pure_rootfs/ $TMP_PATH/
sync

ROOTDIR=`pwd`

FS_TARGET=$TMP_PATH/pure_rootfs
...
if [ "$4" = "RT" ]; then
    ...
    if [ "$1" = "ECU1155" ]; then
        ...
        MODULE_DIR=$FS_TARGET/lib/modules/4.4.189
		ARCH=arm64
...
cd $THIRDPARTY_DIR/RS9113.NBZ.NL.GENR.LNX.1.6.3/source/host/ && make ARCH=$ARCH CROSS_COMPILE=$CROSS_COMPILE KERNELDIR=$KERNELDIR DEVICENAME=DEVICE_NAME_$1
...
# RS9116
cd $THIRDPARTY_DIR/RS9116.NB0.NL.GENR.LNX.1.2.24.0013/source/host/ && make ARCH=$ARCH CROSS_COMPILE=$CROSS_COMPILE KERNELDIR=$KERNELDIR DEVICENAME=DEVICE_NAME_$1
if [ -e release/onebox_nongpl.ko ];then
...
cd $THIRDPARTY_DIR/USB-UAPSTA-8801-U16-X86-W14.68.36.p131-C4X14616_B0-MGPL/wlan_src && make ARCH=$ARCH CROSS_COMPILE=$CROSS_COMPILE KERNELDIR=$KERNELDIR DEVICENAME=DEVICE_NAME_$1
if [ -e mlan.ko ];then
