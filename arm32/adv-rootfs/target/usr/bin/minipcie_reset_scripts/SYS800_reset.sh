...
# SYS800 : BusNum = 1(GPIO0_4)  BusNum = 2 (GPIO3_15)
...
minipcie2_power_reset_for_sys800()  # gpio0_4 
{
...
if [ "$HN" == "sys800" ]; then
	...
	if [ "$1" != "PowerReset" ];then
		echo "SYS800 only support Power Reset."
		exit 0
	fi
	...
	if [ "$?" == "0" ];then 
		...
		if [ "$busnum" == "00$2" ];then  # RS9113 is in the same slot as BusNum
			echo "Detected RS9113 or RS9116, the module should not power reset at SYS800 ,exit"
			exit 0  
		fi
	fi 	
	...
	# BusNum = 1 GPIO0_4
	if [ "$2" == "1" ];then
		minipcie2_power_reset_for_sys800
	fi	
