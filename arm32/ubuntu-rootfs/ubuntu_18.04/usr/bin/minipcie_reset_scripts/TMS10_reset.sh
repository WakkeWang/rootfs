...
# TMS10 : BusNum = 1(GPIO0_4)  BusNum = 2 (GPIO3_15)
...
if [ "$HN" == "tms10" ]; then
	...
	if [ "$1" != "PowerReset" ];then
		echo "$HN only support Power Reset."
		exit 0
	fi
	...
	if [ "$?" == "0" ];then 
		...
		if [ "$busnum" == "00$2" ];then  # RS9113 is in the same slot as BusNum
			echo "Detected RS9113 or RS9116, the module should not power reset at TMS10 ,exit"
			exit 0  
		fi
	fi 	
