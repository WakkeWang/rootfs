...
if [ "$HN" == "adam3600ds" ]; then
	...
	if [ "$1" != "PowerReset" ];then
		echo "ADAM3600DS only support Power Reset."
		exit 0
	fi
	...
	if [ "$?" == "0" ];then 
		...
		if [ "$busnum" == "00$2" ];then  # RS9113 is in the same slot as BusNum
			echo "Detected RS9113 or RS9116, the module should not power reset at ADAM3600DS,exit"
			exit 0  
		fi
	fi 	
